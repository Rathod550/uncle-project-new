<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Blogs Edit ') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Edit Blog') }}</flux:subheading>
            ...
            <form wire:submit="submit" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="flex-1 min-w-[200px]">
                    ...
                    <div class="@error('description') border border-red-500 rounded-md @else border border-gray-300 rounded-md @enderror">
                        ...
                        <!-- Hidden field to ensure Livewire binding -->
                        <input type="hidden" wire:model.defer="description">
                    </div>

                    @error('description')
                        <div class="flex items-center text-sm text-red-600 mt-4">
                            <svg class="shrink-0 size-5 inline" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
